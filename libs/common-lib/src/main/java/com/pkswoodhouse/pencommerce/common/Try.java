 ...
 * <ul>
 * <li>{@link Supplier} which is an extension of {@link java.util.function.Supplier} allowing to throw any exception</li>
 * <li>{@link Runnable} which is an extension of {@link java.lang.Runnable} allowing to throw any exception</li>
 * </ul>
 ...
 * </pre>
 */
sealed public interface Try<T> permits Try.TryVoid, Try.TryValue {
    ...
    sealed interface TryValue<T> extends Try<T> permits Value, Failure {
        default <U> TryValue<U> map(java.util.function.Function<T, U> mapper) {
            return switch (this) {
                case Value<T>(var value) -> Try.of(() -> mapper.apply(value));
                case Failure failure -> failure;
            };
        }
        
        default <U> TryValue<U> flatMap(java.util.function.Function<T, TryValue<U>> mapper) {
            return switch (this) {
                case Value<T>(var value) -> mapper.apply(value);
                case Failure failure -> failure;
            };
        }
        ...
        default T getOrElse(T defaultValue) {
            return switch (this) {
                ...
                case Failure ignored -> defaultValue;
            };
        }
        
        default T getOrElse(java.util.function.Supplier<T> defaultSupplier) {
            return switch (this) {
                case Value<T>(var value) -> value;
                case Failure ignored -> defaultSupplier.get();
            };
        }
        
        default Either<Exception, T> toEither() {
            return switch (this) {
                case Value<T>(var value) -> Either.right(value);
                case Failure(var ex) -> Either.left(ex);
            };
        }
        
        default Either<String, T> toEitherWithMessage() {
            return switch (this) {
                case Value<T>(var value) -> Either.right(value);
                case Failure(var ex) -> Either.left(ex.getMessage());
            };
        }
    }
